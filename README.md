...
| 4            | Train_001      | 4          | EfficientNetB0 | クラスごとに層化交差検証         | 回転・シフト         | なし                  | 0.50615 | 0.50968 | 322/22.4          | 
...
結果は、一番結果の良いフォールド(実験インデックス4)で上位22.4%であった。  

また、フォールドでの予測結果を単純平均とパブリックリーダーボードの結果を元にして加重平均でアンサンブルした。  
結果は、単純平均で上位15.3%、加重平均で上位15.1%であった。 

その後、推論時にデータ拡張（TTA）を行ったときアンサンブル効果でモデルの性能が良くなるため、TTAを行った。  
結果は、各フォールドで全体的に良くなったが、単純平均・加重平均アンサンブルした際に上位16.8%であり、アンサンブルでは性能低下が確認された。  
...
結果は、単純平均で上位9.3%、加重平均で上位9.2%でTrain_001より性能向上が確認された。
...
結果は、Train_001より全体的に性能低下が確認（実験インデックス22~27）されたため、
...
結果は、Train_001より全体的に性能低下が確認（実験インデックス28~33）されたため、
データ拡張手法でCutMixを採用しないことにした。  

入賞ソリューションでは、画像を合成することで、性能向上が確認されたと書いてあったが、  
CutMixで座標情報位置をランダムに選択していたため、モデルの性能が向上しなかったと思われる。  
おそらく座標位置なども考慮してCutMixを行っていれば、性能向上していたと思われる。  

今回は、時間の制限もあるため、画像の合成やCutMixの改良などは行っていない。

---

**Train_005.ipynb**  
Train_001と同様だが、モデルをResNet50に変更した。  
ResNet50で学習した目的は、最終的にモデルをアンサンブルするためである。  

結果は、単純平均で上位13.5%であった。
...
結果は、単純平均で上位13.0%であり、Train_005より性能向上が確認された。。
...
Train_002で学習した半教師あり学習の加重平均のアンサンブル結果の、上位9.2%よりも上位の順位に着くことができた。

## 今後
上位ソリューションでは、物体検出でドライバーが写っている領域を切り出して、学習するなどの工夫が見られた。  
今回の実験では、データ拡張手法の拡大(zoom)がそれにあたり、性能向上が確認されたと考えられる。  

また、画像の合成なども有効な手法であり、今回の実験ではCutMixがそれあたるが、切り出しの際の位置情報を考慮しなかったため性能向上は確認されなかった。  

今後、もう一度、本コンペに取り組む場合、物体検出手法や、画像の合成、複数モデルのアンサンブルを行い、更なる性能向上を目指したい。
